...
### User Story 1 — Capture osehravista Docker M-Unit Baseline (Priority: P1)

A developer wants to establish the ground truth for VistA's existing M-Unit test suite by running all 38 test routines (~1,204 assertions) against the live osehravista Docker container. This captures which tests pass and which fail in the original MUMPS environment, so the transpiled Python is only expected to match — not exceed — the original behavior.
...
**Independent Test**: Can be fully tested by starting the osehravista Docker container, running the baseline tool, and verifying the output JSON contains results for all 38 test routines across 6 packages.

**Acceptance Scenarios**:

1. **Given** the osehravista Docker container is running and accessible via SSH, **When** the baseline runner executes all M-Unit test routines, **Then** structured JSON results are produced containing per-routine pass/fail counts for all 38 routines across 6 packages (MASH Utilities, M XML Parser, VA FileMan, Problem List, Scheduling, Registration).
2. **Given** a test routine must be imported before execution, **When** the baseline runner encounters a routine not already loaded in osehravista, **Then** it imports the routine from the VistA submodule (`vista-test/VistA/Packages/*/Testing/MUnit/*.m`) before running it.
3. **Given** some M-Unit tests may fail even on the original osehravista, **When** a test routine produces failures or errors, **Then** the baseline records failure counts and individual failure messages (entry tag, test name, assertion message) so they can inform expected-failure annotations later.
4. **Given** the baseline has been captured, **When** it is saved to the designated location and committed, **Then** it can be loaded by the pytest adapter in Story 3 to drive `xfail` annotations without requiring a running osehravista Docker container.
...
A developer wants to run the same M-Unit test routines — transpiled to Python — against the transpiled VistA codebase and compare results to the osehravista baseline. Tests that pass on osehravista must also pass in Python; tests that fail on osehravista are marked as expected failures.
...
2. **Given** a test routine passes on osehravista, **When** the same routine is transpiled and run in Python, **Then** it also passes (same number of checks, zero failures, zero errors).
3. **Given** a test routine fails on osehravista (recorded in the baseline), **When** that routine is run in Python, **Then** it is marked as `xfail` (expected failure) rather than a test failure.
4. **Given** the full set of 38 test routines, **When** `uv run pytest tests/vista/munit/` is run, **Then** each routine appears as a separate test item with clear pass/xfail/fail reporting.
...
A developer needs to populate the m2py global store with data from osehravista so that transpiled test routines can access the globals they depend on (data dictionaries, patient records, etc.).
...
**Independent Test**: Can be tested by exporting globals from osehravista, importing into the m2py global store, and querying specific expected globals (e.g., `^DD(2,0)` exists, `^DPT("B",...)` has entries).

**Acceptance Scenarios**:

1. **Given** the osehravista Docker container is running, **When** the bootstrap tool exports globals required by M-Unit test packages, **Then** the exported data covers at minimum: `^DD` (data dictionary), `^DIC` (file attributes), and any package-specific globals identified in the test plan.
2. **Given** exported global data in a portable format, **When** the bootstrap tool imports it into the m2py global store, **Then** transpiled code accessing those globals via `MDict` retrieves the same values as the original MUMPS code on osehravista.
...
4. **Given** the M-Unit self-tests (`%utt1`–`%utt7`, `%uttcovr`) use only the framework's own globals, **When** those tests run with minimal bootstrap, **Then** they validate the M-Unit framework transpilation works correctly.
...
1. **Given** the M-Unit self-tests (`%utt1`–`%utt7`, `%uttcovr`, 8 routines) have no external data dependencies, **When** they are the first tests transpiled and run, **Then** they validate that the M-Unit framework itself works correctly in Python.
...
3. **Given** VA FileMan tests (5 routines) require `^DD` and test fixture files, **When** they are run after global bootstrap is complete, **Then** they validate date/time, lookup, sort, and computed field operations.
4. **Given** Problem List (8 routines), Scheduling (12 routines), and Registration (1 routine) tests require patient and clinic data, **When** they are run with full global bootstrap, **Then** they validate clinical infrastructure APIs.

---

### Edge Cases

- What happens when the osehravista Docker container is not running or SSH is unreachable? The baseline runner must report a clear error and not produce partial results.
...
- What happens when a test routine imports successfully but its dependencies are missing from osehravista? The baseline must record this as an error, not silently skip the routine.
...
- **FR-001**: The baseline runner MUST connect to the osehravista Docker container via SSH programmer mode and enter MUMPS direct mode.
- **FR-002**: The baseline runner MUST import M-Unit test routines from the VistA submodule (`vista-test/VistA/Packages/*/Testing/MUnit/*.m`) into the osehravista instance before executing them.
...
- **FR-005**: The baseline runner MUST produce a structured JSON file containing per-routine results (test count, failure count, error count, individual failure details) grouped by package. This file is committed into the vista-test repository and regenerated manually when osehravista or test routines change.
...
- **FR-015**: The pytest adapter MUST compare transpiled test results against the osehravista baseline: routines that pass on osehravista must pass in Python; routines that fail on osehravista are marked `xfail`.
...
#### Global State Bootstrap (Phase 0b — stretch goal; ZWR parsing in m2py, fixtures in vista-test)

- **FR-018**: *(Stretch goal)* The ZWR parser/serializer MUST live in `src/m2py/runtime/zwr.py` as a general-purpose capability for any `GlobalStorageBackend`. The bootstrap tool MUST export required globals from the osehravista Docker container in ZWR format.
- **FR-019**: *(Stretch goal)* The `import_zwr()` function MUST import ZWR-format globals into any m2py global store backend so transpiled code can access them.
...
- **FR-024**: All VistA-specific functional test code (baseline runner, pytest adapter, bootstrap fixtures, global capture scripts) MUST reside in the `vista-test` repository. General-purpose ZWR import/export and CLI support live in m2py.
...
- **VEHU Baseline**: *(historical name)* See "osehravista Baseline" below. Originally captured from the WorldVistA VEHU Docker image; switched to osehravista during research because VEHU was missing production routines.
- **osehravista Baseline**: A JSON file containing per-routine test results from running M-Unit tests against the osehravista Docker container. Serves as the ground truth for comparing transpiled Python behavior.
- **Global State Snapshot**: An export of MUMPS globals from osehravista (in ZWR format) required by test routines. Loaded into the m2py global store before running transpiled tests.

## Clarifications

### Session 2026-02-19

- Q: Does "Phase 0 complete" mean all 38 routines pass, or a subset? → A: Minimum viable = Tier 1+2 (12 routines: %utt1-7, %uttcovr + 4 XML Parser). Tiers 3-4 (FileMan, Problem List, Scheduling, Registration) are stretch goals within the same spec.
...
- The osehravista Docker image is available and can be pulled/started via Docker. *(Originally planned for WorldVistA VEHU; switched to osehravista during research because VEHU was missing production routines like MXMLTMP1, MXMLPATH, MXMLTMPL.)*
- SSH access to the osehravista container works for programmer mode.
- The `%ut` / `%ut1` M-Unit framework is already installed in the osehravista instance.
- Test routines from the VistA submodule are compatible with the osehravista instance.
- The m2py transpiler successfully transpiles the M-Unit framework routines (`%ut`, `%ut1`) and all 38 test routines (99.99% transpile rate applies).
- Global export from osehravista via `%GO` or equivalent is feasible for the required data sets.
...
- **SC-001**: osehravista baseline captured for all 38 M-Unit test routines, with per-routine results stored in a structured format that the pytest adapter can load.
- **SC-002**: The M-Unit output parser correctly extracts results from 100% of M-Unit output variants encountered in the 38 test routines, validated by unit tests against captured sample output.
- **SC-003**: The M-Unit self-tests (`%utt1`–`%utt7`, `%uttcovr`, 8 routines) pass when transpiled and run in Python, validating framework correctness.
...
- **SC-005**: Every Tier 1+2 M-Unit test routine (12 routines: `%utt1`–`%utt7`, `%uttcovr` + 4 M XML Parser) that passes on osehravista also passes when transpiled and run in Python — zero regressions compared to the baseline. Tiers 3-4 (FileMan, Problem List, Scheduling, Registration) are stretch goals tracked in the same spec.
...
- **SC-007**: Running `uv run pytest tests/vista/munit/` from the vista-test directory discovers and executes all 38 transpiled test routines with clear per-routine pass/xfail/fail reporting.
