...
class ScopeVariables:
    ...
    formal_params: Set[str] = field(default_factory=set)  # Formal parameter names (implicit NEW)
...
def _extract_expression_variables(expr) -> Set[str]:
    ...
    from ..asg.expressions import (
        MVariable, MBinaryOp, MUnaryOp, MLiteral, 
        MIntrinsicFunction, MExtrinsicFunction, MSpecialVariable
    )
...
def check_requires_runtime_scope(label: MLabel) -> bool:
    ...
    """
    from ..asg.expressions import MIndirection
...
def compute_function_signature(label: MLabel, scope_vars: ScopeVariables) -> FunctionSignature:
...
def compute_transitive_outputs(
    ...
    transitive_outputs = {
        name: set(vars.output_variables)
        for name, vars in label_vars.items()
    }
    ...
    while changed and iterations < max_iterations:
        ...
        for label_name, calls in call_info.items():
            ...
            for callee_name, call_obj in calls:
                ...
                callee_outputs = transitive_outputs.get(callee_name, set())
                ...
                for binding in bindings:
                    if binding.passing_mode == PassingMode.BY_REFERENCE:
                        ...
                        if binding.formal_name in callee_vars.writes:
                            if binding.caller_var_name and binding.caller_var_name not in current:
