...
def import_zwr(
    ...
    Delegates to ``backend.import_zwr(source)``, which each backend can
    override with a native fast-path (e.g. MUPIP LOAD for YottaDB,
    transaction batching for IRIS).
    ...
    """
    return backend.import_zwr(source)
