...
from m2py.asg.statements import (
    MSetStatement, MWriteStatement, MDoStatement, MGotoStatement,
    MForStatement, MIfStatement, MElseStatement, MQuitStatement,
    MNewStatement, MKillStatement, MHangStatement, MHaltStatement,
    MXecuteStatement, MLockStatement, MMergeStatement, MViewStatement,
    MReadStatement, MOpenStatement, MCloseStatement, MUseStatement,
    MDoBlockStatement, MJobStatement, MBreakStatement,
)
from m2py.asg.expressions import (
    MIntrinsicFunction, MExtrinsicFunction, MVariable, MGlobal,
    MSpecialVariable, MIndirection, MBinaryOp, MUnaryOp, MLiteral,
    MPatternMatch, MNakedGlobal,
)
...
class FileMetrics:
    ...
    def to_dict(self) -> dict:
        ...
        for key in ['intrinsic_functions', 'extrinsic_functions', 'special_variables',
                    'unknown_commands', 'unknown_functions']:
...
KNOWN_INTRINSICS = {
    '$ASCII', '$A', '$CHAR', '$C', '$DATA', '$D', '$EXTRACT', '$E',
    '$FIND', '$F', '$FNUMBER', '$FN', '$GET', '$G', '$JUSTIFY', '$J',
    '$LENGTH', '$L', '$NAME', '$NA', '$NEXT', '$N', '$ORDER', '$O',
    '$PIECE', '$P', '$QLENGTH', '$QL', '$QSUBSCRIPT', '$QS', '$QUERY', '$Q',
    '$RANDOM', '$R', '$REVERSE', '$RE', '$SELECT', '$S', '$STACK', '$ST',
    '$TEXT', '$T', '$TRANSLATE', '$TR', '$VIEW', '$V',
    # Additional common ones
    '$ZBITAND', '$ZBITCOUNT', '$ZBITFIND', '$ZBITGET', '$ZBITNOT',
    '$ZBITOR', '$ZBITSET', '$ZBITSTR', '$ZBITXOR', '$ZCONVERT', '$ZCVT',
    '$ZDATE', '$ZD', '$ZDATEH', '$ZDH', '$ZHEX', '$ZH', '$ZLENGTH', '$ZL',
    '$ZPIECE', '$ZP', '$ZSEARCH', '$ZSORT', '$ZSTRIP', '$ZTIME', '$ZT',
    '$ZTIMEH', '$ZTH', '$ZWIDTH', '$ZW',
    # More Z functions
    '$ZBOOLEAN', '$ZCRC', '$ZINCREMENT', '$ZINCRMT', '$ZSOCKET',
    '$ZSUBSTR', '$ZTRNLNM', '$ZZWRITE',
...
KNOWN_SPECIAL_VARS = {
    '$DEVICE', '$D', '$ECODE', '$EC', '$ESTACK', '$ES', '$ETRAP', '$ET',
    '$HOROLOG', '$H', '$IO', '$I', '$JOB', '$J', '$KEY', '$K',
    '$PRINCIPAL', '$P', '$QUIT', '$Q', '$REFERENCE', '$R', '$STACK', '$ST',
    '$STORAGE', '$S', '$SYSTEM', '$SY', '$TEST', '$T', '$TLEVEL', '$TL',
    '$TRESTART', '$TR', '$X', '$Y', '$ZA', '$ZB', '$ZCMDLINE',
    '$ZDEVICE', '$ZDIRECTORY', '$ZEESSION', '$ZERROR', '$ZE',
    '$ZGBLDIR', '$ZHOROLOG', '$ZH', '$ZINTERRUPT', '$ZIO', '$ZJOB', '$ZJ',
    '$ZKEY', '$ZK', '$ZLEVEL', '$ZL', '$ZMAXTPTIME', '$ZMODE', '$ZM',
    '$ZNAME', '$ZN', '$ZPATNUMERIC', '$ZPN', '$ZPOSITION', '$ZPOS',
    '$ZPROMPT', '$ZPR', '$ZREFERENCE', '$ZR', '$ZROUTINES', '$ZRO',
    '$ZSOURCE', '$ZS', '$ZSTATUS', '$ZST', '$ZSYSTEM', '$ZSYS', '$ZTDATA',
    '$ZTIMEOUT', '$ZTLEVEL', '$ZTOLDVAL', '$ZTRAP', '$ZTSTEP', '$ZTVALUE',
    '$ZUSEDSTOR', '$ZVERSION', '$ZV', '$ZYERROR',
}


def find_vista_files(package_filter: Optional[str] = None, limit: Optional[int] = None) -> list[Path]:
...
def analyze_asg(routine: MRoutine, metrics: FileMetrics) -> None:
    ...
    def walk_statements(scope, depth=0):
        ...
        for stmt in scope.statements:
            ...
            metrics.statement_types[stmt_type] = metrics.statement_types.get(stmt_type, 0) + 1
...
def analyze_expressions_in_stmt(stmt: Any, metrics: FileMetrics) -> None:
    ...
    def walk_expr(expr):
        ...
        if isinstance(expr, MIntrinsicFunction):
            ...
            func_name_with_prefix = f"${func_name}" if not func_name.startswith('$') else func_name
            metrics.intrinsic_functions.add(func_name_with_prefix)
            # Check if unknown (need to compare with $ prefix)
            if func_name_with_prefix.upper() not in KNOWN_INTRINSICS and not func_name_with_prefix.upper().startswith('$Z'):
    ...
    for attr in ['condition', 'value', 'arguments', 'targets', 'postcondition',
                 'device_expr', 'timeout', 'parameters', 'expressions']:
...
def evaluate_file(filepath: Path, parser: MUMPSParser, verbose: bool = False) -> FileMetrics:
    ...
    try:
        ...
        if verbose:
            print(f"✓ {filepath.name}: {metrics.label_count} labels, {metrics.statement_count} stmts")
...
def run_evaluation(
    ...
    for i, filepath in enumerate(files):
        ...
        if metrics.parsed:
            ...
            for func in metrics.intrinsic_functions:
                results.intrinsic_function_usage[func] = results.intrinsic_function_usage.get(func, 0) + 1
            
            # Aggregate extrinsic functions
            for func in metrics.extrinsic_functions:
                results.extrinsic_function_usage[func] = results.extrinsic_function_usage.get(func, 0) + 1
            
            # Aggregate special variables
            for var in metrics.special_variables:
                results.special_variable_usage[var] = results.special_variable_usage.get(var, 0) + 1
            
            # Aggregate statement types
            for stmt_type, count in metrics.statement_types.items():
                results.statement_type_totals[stmt_type] = results.statement_type_totals.get(stmt_type, 0) + count
            ...
            for func in metrics.unknown_functions:
                results.unknown_functions[func] = results.unknown_functions.get(func, 0) + 1
            
        else:
            results.parse_failures += 1
            results.errors_by_type[metrics.error_type] = results.errors_by_type.get(metrics.error_type, 0) + 1
    ...
    results.most_complex_files = [
        {'file': m.filepath, 'package': m.package, 'score': m._complexity,
         'gotos': m.goto_count, 'fors': m.for_loop_count, 'nested_for': m.nested_for_count,
         'xecute': m.xecute_count, 'indirection': m.indirection_count}
...
def print_summary(results: EvaluationResults) -> None:
    ...
    print(f"Successfully parsed:   {results.parsed_successfully} ({100*results.parsed_successfully//results.total_files}%)")
    print(f"Parse failures:        {results.parse_failures} ({100*results.parse_failures//results.total_files}%)")
    ...
    if results.statement_type_totals:
        print("\n--- Statement Types ---")
        for stmt_type, count in sorted(results.statement_type_totals.items(), key=lambda x: -x[1])[:20]:
            print(f"  {stmt_type}: {count:,}")
    
    if results.intrinsic_function_usage:
        print("\n--- Top Intrinsic Functions ---")
        for func, count in sorted(results.intrinsic_function_usage.items(), key=lambda x: -x[1])[:15]:
            print(f"  {func}: {count:,}")
    
    if results.special_variable_usage:
        print("\n--- Top Special Variables ---")
        for var, count in sorted(results.special_variable_usage.items(), key=lambda x: -x[1])[:15]:
            print(f"  {var}: {count:,}")
    
    if results.unknown_functions:
        print("\n--- Unknown Functions (potential issues) ---")
        for func, count in sorted(results.unknown_functions.items(), key=lambda x: -x[1])[:20]:
            print(f"  {func}: {count}")
    
    if results.most_complex_files:
        print("\n--- Most Complex Files (top 20) ---")
        for i, f in enumerate(results.most_complex_files[:20], 1):
            print(f"  {i:2}. {Path(f['file']).name}: score={f['score']:.1f} "
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate M2PY parser against VistA-M codebase")
    ...
    parser.add_argument('--sample', action='store_true', help="Quick sample run (100 files)")
    parser.add_argument('--report', action='store_true', help="Generate full report and save")
