...
def profile_file(parser: MUMPSParser, filepath: Path) -> Tuple[float, float, float, int, int]:
...
def main():
    parser = argparse.ArgumentParser(description="Profile variable analysis performance")
    ...
    parser.add_argument("--detailed", action="store_true", help="Show per-file breakdown")
    ...
    for i, filepath in enumerate(files):
        ...
        if args.detailed and not args.summary:
            ...
            print(f"{filepath.name:40} {lines:5} lines {labels:4} labels "
    ...
    print(f"{'Metric':<20} {'Mean':>10} {'Median':>10} {'Std Dev':>10} {'Max':>10} {'Total':>10}")
    print("-" * 70)
    
    for name, times in [("Parse", parse_times), ("Analyze", analyze_times), 
                         ("Signatures", sig_times), ("Total", total_times)]:
        if times:
            ...
            print(f"{name:<20} {mean:>10.2f} {median:>10.2f} {stdev:>10.2f} {max_t:>10.2f} {total:>10.0f}")
    ...
    files_over_500 = [(f, p + a + s, l) for f, p, a, s, l, lb in results if l >= 500]
    if files_over_500:
