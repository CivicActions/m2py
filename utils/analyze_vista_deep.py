...
from m2py.asg.statements import (
    ...
    MNewStatement, MKillStatement, MHaltStatement,
    MXecuteStatement, MLockStatement, MDoBlockStatement, MJobStatement,
)
...
def analyze_scope(scope, result: DeepAnalysisResult, in_for: bool, for_depth: int) -> None:
    ...
    for stmt in scope.statements:
        ...
        if isinstance(stmt, MForStatement):
            ...
            if stmt.then_scope:
                analyze_scope(stmt.then_scope, result, in_for=in_for, for_depth=for_depth)
    ...
    def walk_expr(expr, depth=0):
        ...
        if isinstance(expr, MIndirection):
            ...
            if hasattr(expr, 'name'):
                ...
                unusual = ['$ZBITAND', '$ZBITCOUNT', '$ZBITFIND', '$ZBOOLEAN',
                          '$ZSOCKET', '$ZTRNLNM', '$ZGETJPI']
...
def find_files(package_filter: Optional[str] = None, limit: Optional[int] = None) -> list[Path]:
...
def main():
    ...
    parser_arg.add_argument('--show-all', action='store_true', help="Show all files, not just interesting ones")
