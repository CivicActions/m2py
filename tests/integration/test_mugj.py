...
class TestV1FORBForPatterns:
    ...
    def test_v1forb_has_expected_labels(self, mugj_inref_dir):
        ...
        assert any(name.isdigit() for name in label_names), (
            "V1FORB should have numeric labels"
        )
            ...
            for_count = len(
                [
                    ...
                    for m in __import__("re").finditer(
                        r"\bFOR?\s+", line, __import__("re").IGNORECASE
                    )
                ]
            )
            if for_count >= 2:
...
class TestQuitExitPointDetection:
    ...
    def test_v1forb_has_open_ended_with_quit(self, mugj_file):
        ...
        assert len(open_ended_with_quit) > 0, (
            "V1FORB should have open-ended FOR with QUIT"
        )
...
class TestV1FORC2NestedForGoto:
    ...
    def test_v1forc2_for_with_goto_in_body(self, mugj_file):
        ...
        assert len(for_and_goto_lines) > 0, (
            "V1FORC2 should have lines with FOR and GOTO"
        )
...
class TestParseAllMUGJFiles:
    ...
    def test_parse_all_mugj_files(self, mugj_inref_dir):
        ...
        if failed_files:
            fail_report = "\n".join(
                f"  - {name}: {err}" for name, err in failed_files[:10]
            )
        ...
        # Success assertion
        assert success_count + skipped == total, (
            f"Parsed {success_count}, skipped {skipped} of {total} files"
        )
        assert success_count >= 370, (
            f"Expected at least 370 MUGJ files, got {success_count}"
        )
    ...
    def test_mugj_file_count(self, mugj_inref_dir):
        ...
        assert len(real_files) >= 370, (
            f"Expected 370+ MUGJ files, found {len(real_files)}"
        )
        assert len(all_files) >= 375, (
            f"Expected 375+ total MUGJ files, found {len(all_files)}"
        )
            ...
            if "F I=1:1:9" in line:
                ...
                if cmds:
                    ...
                    assert param.start.value == 1, (
                        f"start value should be 1, got {param.start.value}"
                    )
                    assert param.step.value == 1, (
                        f"step value should be 1, got {param.step.value}"
                    )
                    assert param.end.value == 9, (
                        f"end value should be 9, got {param.end.value}"
                    )
                    return
                ...
                if stmt:
                    ...
                    for target in stmt.targets:
                        if hasattr(target, "name") and target.name:
                            assert "<textx:" not in target.name, (
                                f"Target name corrupted: {target.name}"
                            )
                        if hasattr(target, "routine") and target.routine:
                            assert "<textx:" not in target.routine, (
                                f"Routine corrupted: {target.routine}"
                            )

        assert goto_count > 0, "No GOTO statements parsed from V1GO1.m"
...
class TestV1FNIntrinsicFunctions:
    ...
    def test_all_v1fn_files_parse(self, mugj_inref_dir):
        ...
        assert parsed_count >= 9, (
            f"Expected at least 9 V1FN files, found {parsed_count}"
        )
...
class TestControlFlowBodyIntegration:
    ...
    def test_v1ie1_if_bodies_populated(self, parser):
        ...
        assert if_with_body > 0, (
            "V1IE1.m IF statements should have populated then_scope"
        )
...
class TestMUGJValidation:
    ...
    def test_vv2cs_multi_for_with_postconditions(self, parser, mugj_inref_dir):
        ...
        assert len(for_stmts) >= 2, (
            f"Label 5 should have at least 2 FOR statements, got {len(for_stmts)}"
        )
        ...
        assert len(for1.body.statements) >= 2, (
            "First FOR should have QUIT and DO in body"
        )
        quit_stmt = for1.body.statements[0]
        assert isinstance(quit_stmt, MQuitStatement), "First body stmt should be QUIT"
        assert quit_stmt.postcondition is not None, (
            "QUIT should have postcondition (Q:I=10)"
        )
        ...
        assert len(for2.body.statements) >= 2, (
            "Second FOR should have QUIT and DO in body"
        )
        quit_stmt2 = for2.body.statements[0]
        assert isinstance(quit_stmt2, MQuitStatement), "First body stmt should be QUIT"
        assert quit_stmt2.postcondition is not None, (
            "QUIT should have postcondition (QUIT:I=11)"
        )
    ...
    def test_vv2fn1_naked_global_kill(self, parser, mugj_inref_dir):
        ...
        kill_stmts = [
            s for s in label70.body.statements if isinstance(s, MKillStatement)
        ]
        assert len(kill_stmts) >= 2, (
            f"Label 70 should have at least 2 KILL statements, got {len(kill_stmts)}"
        )
        ...
        assert hasattr(first_target, "name"), (
            "First KILL target should be GlobalVariable with name"
        )
...
class TestVariableAnalysisMUGJ:
    ...
    def test_v1nx2_exclusive_new(self, parser, mugj_inref_dir):
        ...
        _has_exclusive_new = False  # Flag tracked for analysis, assertion commented out
        for label in routine.labels:
            for stmt in label.body.walk_statements():
                if hasattr(stmt, "exclusive") and stmt.exclusive:
                    _has_exclusive_new = True
                    break
    ...
    def test_scope_strategy_classification_integration(self, parser, mugj_inref_dir):
        ...
        assert (
            ScopeStrategy.SUBROUTINE in strategies
            or ScopeStrategy.PURE_FUNCTION in strategies
            or ScopeStrategy.FUNCTION_WITH_OUTPUTS in strategies
