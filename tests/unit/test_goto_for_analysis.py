...
from m2py.asg.statements import (
    MGotoStatement, MDoStatement, MForStatement, MSetStatement,
    MForParameter, MAssignment
)
from m2py.asg.expressions import MVariable, MLiteral
from m2py.asg.enums import GotoType, ForLoopType, ForParamType, LiteralType
...
"""
    ...
    def test_v1fora1_analysis(self):
        ...
        assert len(step_zero_fors) >= 5, f"Expected >=5 step-0 FORs, got {len(step_zero_fors)}"
        assert len(infinite_fors) >= 5, f"Expected >=5 infinite FORs, got {len(infinite_fors)}"
        
        # Check that GOTOs inside FORs are detected
        fors_with_gotos = [
            (label.name, stmt) for label in routine.labels 
            for stmt in label.body.statements 
