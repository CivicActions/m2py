...
Migrated from:
- tests/unit/test_io_commands.py::TestMergeCommand
- tests/unit/test_multi_arg_commands.py::TestMultiMerge
"""
...
class TestMergeCommandAnalysis:
    ...
    def test_merge_command_simple(self):
        """MERGE dest=source produces MMergeStatement with both fields (§8.2.13).

        Migrated from: test_io_commands.py::TestMergeCommand::test_merge_command_simple
        """
        parser = MUMPSParser()
    ...
    def test_merge_command_with_postcondition(self):
        """MERGE:condition dest=source handles postcondition (§8.2.13).

        Migrated from: test_io_commands.py::TestMergeCommand::test_merge_command_with_postcondition
        """
        parser = MUMPSParser()
        routine = parser.parse("TEST\n M:X>0 ^DEST=^SRC\n")
    ...
    def test_merge_with_local_variables(self):
        """MERGE can merge local variable trees (§8.2.13).

        Migrated from: test_io_commands.py::TestMergeCommand::test_merge_with_local_variables
        """
        parser = MUMPSParser()
        routine = parser.parse("TEST\n M LOCAL1=LOCAL2\n")
...
@pytest.mark.asg
class TestMultiMergeAnalysis:
    """Tests for MERGE command with multiple merge pairs (§8.2.13).

    Migrated from: tests/unit/test_multi_arg_commands.py::TestMultiMerge
    """

    def test_single_merge(self):
        """Single MERGE pair works correctly (§8.2.13)."""
        parser = MUMPSParser()
        routine = parser.parse("TEST\n M X=Y\n")
    ...
    def test_multi_merge(self):
        """MERGE with multiple pairs captures all pairs (§8.2.13).
    ...
    def test_multi_merge_globals(self):
        """MERGE with multiple global pairs (§8.2.13).
    ...
    def test_vista_ztmon_pattern(self):
        """Test VistA ZTMON.m pattern with two merge pairs (§8.2.13).

        This pattern appears in 210+ VistA files.
        Migrated from: test_multi_arg_commands.py::TestMultiMerge::test_vista_ztmon_pattern
        """
        parser = MUMPSParser()
