...
Transactions provide atomic database operations via the `GlobalStorageBackend` protocol:

```python
# Begin transaction (increments $TLEVEL)
rt.globals.transaction_start()

# Commit transaction (decrements $TLEVEL, persists if 0)
rt.globals.transaction_commit()

# Rollback transaction (reverts changes, sets $TLEVEL=0)
rt.globals.transaction_rollback()

# Get current nesting level
level = rt.globals.get_tlevel()  # 0 = no transaction
```
...
_rt.globals.set("A", (), "1")
_rt.globals.transaction_commit()

# MUMPS: W $TLEVEL
_rt.write(_rt.tlevel())
```

## Lock Operations

Resource locking for concurrent access control via the `GlobalStorageBackend` protocol:

```python
# Acquire lock (incremental)
success = rt.globals.lock("DATA", ("1", "2"), timeout=5, lock_type="+")

# Release lock (decremental)
rt.globals.lock("DATA", ("1", "2"), timeout=None, lock_type="-")

# Release all locks
rt.globals.unlock_all()
```

**Generated Code:**
```python
# MUMPS: L +^DATA(1,2):5 W $T
_test = _rt.globals.lock('DATA', ('1', '2'), 5, '+')
_rt.write(int(_test))

# MUMPS: L -^DATA
_rt.globals.lock('DATA', (), None, '-')

# MUMPS: L  (argumentless - release all)
_rt.globals.unlock_all()
```

**$TEST Behavior:**
- Timed LOCK sets `_test` to the success result (True/False)
- Untimed LOCK does not modify `_test`
- LOCK - (release) with timeout always sets `_test = True`
